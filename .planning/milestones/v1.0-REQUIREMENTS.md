...
**Shipped:** 2026-04-26
**Final status:** all 47 requirements satisfied. See `.planning/milestones/v1.0-ROADMAP.md` for the per-phase mapping and `.planning/v1.0-MILESTONE-AUDIT.md` for the cross-phase coverage matrix.

**Carried to v1.1 (deferred from this milestone):**
- Enrichment writes `book.reference_pages` from OL Edition `number_of_pages` (currently the yearly-report falls back to `MAX(book_device.pages)` via COALESCE).
- Bulk-enqueue helper for the 8+ books currently in `enrichment_status='failed'`.
- Orphan author GC.

**Original document below — checkboxes left as-recorded during execution; the audit's status determination matrix is the authoritative completion record.**
